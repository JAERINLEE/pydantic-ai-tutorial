...
- **에이전트** (`src/agent/faq_agent.py`): `FAQDatabase` dataclass가 FAQ + 게시판 데이터를 병합하여 TF-IDF 인덱스(scikit-learn) 구축. `search_faq` 도구가 cosine similarity로 상위 3개 결과를 출처(FAQ/게시판)와 함께 반환. `GoogleModel("gemini-2.5-flash")`와 한국어 시스템 프롬프트 사용.
...
데이터 흐름: (sitemap XML → FAQ 크롤러 → `faq_lineworks.json`) + (게시판 → Playwright 크롤러 → `board_lineworks.json`) → `FAQDatabase.load()` → TF-IDF 인덱스 → 에이전트 도구 → LLM 응답.
